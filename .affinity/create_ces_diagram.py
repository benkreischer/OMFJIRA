#!/usr/bin/env python3
"""
Create CES-Centered Diagram with Proper Naming Convention
Based on the working TOKR script - EXACT COPY with CES substituted for TOKR
"""
...
def create_ces_centered_diagram(relationships_df, output_file='ces_centered_diagram.png', weighted_sum=0):
    """Create a CES-centered radial affinity diagram with ring layout and professional styling."""
    ...
    # Check if CES exists in the graph
    if 'CES' not in G.nodes():
        print("Warning: CES project not found in data. Cannot create CES-centered diagram.")
        return None, 0, 0, 0, 0, {}, []

    # Get CES's direct connections
    ces_connections = list(G.neighbors('CES'))
    print(f"CES has {len(ces_connections)} direct connections")

    # Categorize connected projects by their connectivity to OTHER projects in the CES list
    center_hub_connections = []
    center_high_connections = []
    center_medium_connections = []
    center_low_connections = []

    # Calculate connections for each project to other projects in the CES list
    ces_connected_projects = list(ces_connections)
    project_connection_counts = {}
    
    for conn in ces_connections:
        connections = 0
        # Count connections to other projects in the CES list (including CES)
        ...
        for _, row in project_rels.iterrows():
            ...
            # Count connections to other projects in the CES list (including CES)
            if other_project in ces_connected_projects + ['CES']:
                connections += 1
    ...
    print(f"CES filtered connections: {len(center_hub_connections + center_high_connections + center_medium_connections + center_low_connections)}")
    ...
    if edges_to_draw:
        # Separate CES hub edges from all other edges
        ces_hub_edges = []
        other_edges = []
        ...
        if ces_hub_edges:
            nx.draw_networkx_edges(G, pos, edgelist=ces_hub_edges, alpha=1.0, edge_color='#ff8c00', width=1.0)

    # Draw nodes by category with proper sizing (from yesterday's thresholds)
    # Center (CES) - Fixed size 4000, 10% transparent
    if 'CES' in pos:
        nx.draw_networkx_nodes(G, pos, nodelist=['CES'],
    ...
    for node in G.nodes():
        if node in pos:
            ...
            if node == 'CES':
                # CES shows "Customer Experience Services" instead of the project key
                plt.text(x, y+0.05, "Customer Experience Services", ha='center', va='center', fontsize=16, weight='bold', color='black')
            else:
    ...
    for node in G.nodes():
        if node in pos:
            x, y = pos[node]
            if node == 'CES':
                # CES shows the count of projects it's connected to on the chart
                count = str(len(ces_connections))
            else:
                # Connected projects show their connection count to other projects in the CES list
    ...
    # Add title
    plt.title(f'Customer Experience Services (CES) - {len(ces_connections)} Links\n(Filtered: Unresolved Issues, 90-Day Activity)',
              fontsize=20, fontweight='bold', pad=20)

    # Add legend
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', label='CES (Center)',
    ...
    center_connections = list(G.neighbors('CES'))
    center_neighbors = [(n, G.nodes[n]['link_count']) for n in center_connections]
    
    # Sort by column 2 (circle number) descending, then by column 3 (CES links) descending
    def sort_key(item):
        project = item[0]
        # Get the circle number (column 2)
        circle_number = project_connection_counts.get(project, 1)
        
        # Get CES links (column 3)
        ces_links = 0
        for _, row in relationships_df.iterrows():
            if ((row['ProjectKey'] == 'CES' and row['ConnectedProject'] == project) or 
                (row['ProjectKey'] == project and row['ConnectedProject'] == 'CES')):
                ces_links = row['LinkCount']
                break
        
        return (circle_number, ces_links)  # Positive for ascending order, then reverse
    ...
    for project, connections in center_neighbors:
        # Get the specific link count between CES and this project
        ces_to_project_links = 0
        for _, row in relationships_df.iterrows():
            if ((row['ProjectKey'] == 'CES' and row['ConnectedProject'] == project) or 
                (row['ProjectKey'] == project and row['ConnectedProject'] == 'CES')):
                ces_to_project_links = row['LinkCount']
                break
        ...
        line_text = f"{project:<6s}{circle_number:>6d}{ces_to_project_links:>6d}{total_project_connections:>6d}"
    ...
    for project, connections in center_neighbors:
        ...
        total_ces_links += ces_to_project_links
        total_project_connections += total_project_connections_for_this
    
    # Create totals line: Count of rows, Weighted sum, Total CES links, Total project connections
    totals_line = f"{total_rows:<6d}{weighted_sum:>6d}{total_ces_links:>6d}{total_project_connections:>6d}"
    ...
    return G, len(center_hub_connections), len(center_high_connections), len(center_medium_connections), len(center_low_connections), ces_connections

def main():
    """Main function to generate the CES-centered diagram with proper naming"""
    ...
    print("\n" + "="*60)
    print("Creating CES-Centered Project Affinity Diagram...")
    print("="*60)
    
    # Create the diagram
    temp_filename = 'ces_temp.png'
    G, hub_count, high_count, medium_count, low_count, ces_connections = create_ces_centered_diagram(
    ...
    if G is not None:
        ...
        proper_filename = f'{weighted_sum:04d}_{center_count:04d}_{hub_count:04d}_{high_count:04d}_{medium_count:04d}_{low_count:04d}_CES.png'
        ...
        print(f"\nCES-CENTERED PROJECT AFFINITY ANALYSIS SUMMARY")
        center_connections = list(G.neighbors('CES'))
        print(f"CES Total Direct Connections: {len(center_connections)}")
    
    print(f"\n{'='*60}")
    print(f"CES PROJECT PROCESSING COMPLETED!")
