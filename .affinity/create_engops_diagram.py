#!/usr/bin/env python3
"""
Create ENGOPS-Centered Diagram with Proper Naming Convention
Based on the working TOKR script - EXACT COPY with ENGOPS substituted for TOKR
"""
...
def create_cia_centered_diagram(relationships_df, output_file='cia_centered_diagram.png', weighted_sum=0):
    """Create a ENGOPS-centered radial affinity diagram with ring layout and professional styling."""
    ...
    if 'ENGOPS' not in G.nodes():
        print("Warning: ENGOPS project not found in data. Cannot create ENGOPS-centered diagram.")
        return None, 0, 0, 0, 0, {}, []

    cia_connections = list(G.neighbors('ENGOPS'))
    print(f"ENGOPS has {len(cia_connections)} direct connections")
    ...
    for conn in cia_connections:
        ...
        for _, row in project_rels.iterrows():
            ...
            if other_project in cia_connected_projects + ['ENGOPS']:
                connections += 1
    ...
    print(f"ENGOPS filtered connections: {len(center_hub_connections + center_high_connections + center_medium_connections + center_low_connections)}")
    ...
    if edges_to_draw:
        ...
        for u, v in edges_to_draw:
            if u == 'ENGOPS' or v == 'ENGOPS':
                target_node = v if u == 'ENGOPS' else u
    ...
    if 'ENGOPS' in pos:
        nx.draw_networkx_nodes(G, pos, nodelist=['ENGOPS'],
    ...
    for node in G.nodes():
        if node in pos:
            x, y = pos[node]
            if node == 'ENGOPS':
                plt.text(x, y+0.05, "Engineering Operations", ha='center', va='center', fontsize=16, weight='bold', color='black')
            else:
    ...
    plt.title(f'Engineering Operations (ENGOPS) - {len(cia_connections)} Links\n(Filtered: Unresolved Issues, 90-Day Activity)',
              fontsize=20, fontweight='bold', pad=20)

    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', label='ENGOPS (Center)',
    ...
    center_connections = list(G.neighbors('ENGOPS'))
    ...
    def sort_key(item):
        ...
        cia_links = 0
        for _, row in relationships_df.iterrows():
            if ((row['ProjectKey'] == 'ENGOPS' and row['ConnectedProject'] == project) or 
                (row['ProjectKey'] == project and row['ConnectedProject'] == 'ENGOPS')):
...
def main():
    ...
    print("\n" + "="*60)
    print("Creating ENGOPS-Centered Project Affinity Diagram...")
    print("="*60)
    ...
    if G is not None:
        ...
        proper_filename = f'{weighted_sum:04d}_{center_count:04d}_{hub_count:04d}_{high_count:04d}_{medium_count:04d}_{low_count:04d}_ENGOPS.png'
        ...
        print(f"\nENGOPS-CENTERED PROJECT AFFINITY ANALYSIS SUMMARY")
        center_connections = list(G.neighbors('ENGOPS'))
        print(f"ENGOPS Total Direct Connections: {len(center_connections)}")
    
    print(f"\n{'='*60}")
    print(f"ENGOPS PROJECT PROCESSING COMPLETED!")
