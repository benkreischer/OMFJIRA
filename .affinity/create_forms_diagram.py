#!/usr/bin/env python3
"""
Create FORMS-Centered Diagram with Proper Naming Convention
Based on the working TOKR script - EXACT COPY with FORMS substituted for TOKR
"""
...
def create_cia_centered_diagram(relationships_df, output_file='cia_centered_diagram.png', weighted_sum=0):
    """Create a FORMS-centered radial affinity diagram with ring layout and professional styling."""
    ...
    if 'FORMS' not in G.nodes():
        print("Warning: FORMS project not found in data. Cannot create FORMS-centered diagram.")
        return None, 0, 0, 0, 0, {}, []

    cia_connections = list(G.neighbors('FORMS'))
    print(f"FORMS has {len(cia_connections)} direct connections")
    ...
    for conn in cia_connections:
        ...
        for _, row in project_rels.iterrows():
            ...
            if other_project in cia_connected_projects + ['FORMS']:
                connections += 1
    ...
    print(f"FORMS filtered connections: {len(center_hub_connections + center_high_connections + center_medium_connections + center_low_connections)}")
    ...
    if edges_to_draw:
        ...
        for u, v in edges_to_draw:
            if u == 'FORMS' or v == 'FORMS':
                target_node = v if u == 'FORMS' else u
    ...
    if 'FORMS' in pos:
        nx.draw_networkx_nodes(G, pos, nodelist=['FORMS'],
    ...
    for node in G.nodes():
        if node in pos:
            x, y = pos[node]
            if node == 'FORMS':
                plt.text(x, y+0.05, "Form Management", ha='center', va='center', fontsize=16, weight='bold', color='black')
            else:
    ...
    plt.title(f'Form Management (FORMS) - {len(cia_connections)} Links\n(Filtered: Unresolved Issues, 90-Day Activity)',
              fontsize=20, fontweight='bold', pad=20)

    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', label='FORMS (Center)',
    ...
    center_connections = list(G.neighbors('FORMS'))
    ...
    def sort_key(item):
        ...
        cia_links = 0
        for _, row in relationships_df.iterrows():
            if ((row['ProjectKey'] == 'FORMS' and row['ConnectedProject'] == project) or 
                (row['ProjectKey'] == project and row['ConnectedProject'] == 'FORMS')):
...
def main():
    ...
    print("\n" + "="*60)
    print("Creating FORMS-Centered Project Affinity Diagram...")
    print("="*60)
    ...
    if G is not None:
        ...
        proper_filename = f'{weighted_sum:04d}_{center_count:04d}_{hub_count:04d}_{high_count:04d}_{medium_count:04d}_{low_count:04d}_FORMS.png'
        ...
        print(f"\nFORMS-CENTERED PROJECT AFFINITY ANALYSIS SUMMARY")
        center_connections = list(G.neighbors('FORMS'))
        print(f"FORMS Total Direct Connections: {len(center_connections)}")
    
    print(f"\n{'='*60}")
    print(f"FORMS PROJECT PROCESSING COMPLETED!")
