#!/usr/bin/env python3
"""
Create LAS-Centered Diagram with Proper Naming Convention
Based on the working TOKR script - EXACT COPY with LAS substituted for TOKR
"""
...
def create_cia_centered_diagram(relationships_df, output_file='cia_centered_diagram.png', weighted_sum=0):
    """Create a LAS-centered radial affinity diagram with ring layout and professional styling."""
    ...
    if 'LAS' not in G.nodes():
        print("Warning: LAS project not found in data. Cannot create LAS-centered diagram.")
        return None, 0, 0, 0, 0, {}, []

    cia_connections = list(G.neighbors('LAS'))
    print(f"LAS has {len(cia_connections)} direct connections")
    ...
    for conn in cia_connections:
        ...
        for _, row in project_rels.iterrows():
            ...
            if other_project in cia_connected_projects + ['LAS']:
                connections += 1
    ...
    print(f"LAS filtered connections: {len(center_hub_connections + center_high_connections + center_medium_connections + center_low_connections)}")
    ...
    if 'LAS' in pos:
        nx.draw_networkx_nodes(G, pos, nodelist=['LAS'],
    ...
    for node in G.nodes():
        if node in pos:
            x, y = pos[node]
            if node == 'LAS':
                plt.text(x, y+0.05, "Loan Application Services", ha='center', va='center', fontsize=16, weight='bold', color='black')
            else:
    ...
    plt.title(f'Loan Application Services (LAS) - {len(cia_connections)} Links\n(Filtered: Unresolved Issues, 90-Day Activity)',
              fontsize=20, fontweight='bold', pad=20)

    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', label='LAS (Center)',
    ...
    center_connections = list(G.neighbors('LAS'))
    ...
    def sort_key(item):
        ...
        cia_links = 0
        for _, row in relationships_df.iterrows():
            if ((row['ProjectKey'] == 'LAS' and row['ConnectedProject'] == project) or 
                (row['ProjectKey'] == project and row['ConnectedProject'] == 'LAS')):
...
def main():
    ...
    print("\n" + "="*60)
    print("Creating LAS-Centered Project Affinity Diagram...")
    print("="*60)
    ...
    if G is not None:
        ...
        proper_filename = f'{weighted_sum:04d}_{center_count:04d}_{hub_count:04d}_{high_count:04d}_{medium_count:04d}_{low_count:04d}_LAS.png'
        ...
        print(f"\nLAS-CENTERED PROJECT AFFINITY ANALYSIS SUMMARY")
        center_connections = list(G.neighbors('LAS'))
        print(f"LAS Total Direct Connections: {len(center_connections)}")
    
    print(f"\n{'='*60}")
    print(f"LAS PROJECT PROCESSING COMPLETED!")
