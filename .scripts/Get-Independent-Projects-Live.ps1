# =============================================================================
# GET INDEPENDENT PROJECTS - LIVE API DATA
# =============================================================================
#
...
Write-Host "=== ANALYZING INDEPENDENT PROJECTS - LIVE DATA ===" -ForegroundColor Green
...
try {
    ...
    # =============================================================================
    # STEP 2: Get cross-project links for each project
    # =============================================================================
    ...
    foreach ($project in $AllProjects) {
        ...
        # Initialize with empty links array
        $ProjectLinks[$projectKey] = @()
        
        try {
            # Get all issues in this project that have links
            $JqlQuery = "project = `"$projectKey`" AND issueLink IS NOT EMPTY ORDER BY created DESC"
            ...
            # Remove duplicates and get unique linked projects
            $uniqueLinkedProjects = $ProjectLinks[$projectKey] | Sort-Object -Unique | Where-Object { $_ -ne "" }
            $linkCount = $uniqueLinkedProjects.Count
            
            Write-Host "    $projectKey has $linkCount cross-project links" -ForegroundColor Gray
            
            # Small delay to avoid rate limiting
            Start-Sleep -Milliseconds 100
            
        } catch {
            Write-Warning "    Failed to analyze links for $projectKey : $($_.Exception.Message)"
        }
    }
    ...
    $OutputFile = "Independent-Projects-Live-Analysis.csv"
