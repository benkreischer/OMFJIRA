# =============================================================================
# GET ALL ATTACHMENTS FOR A PROJECT - V2
# =============================================================================
#
# DESCRIPTION: Gets all attachments for all issues in a specified project
# Uses proper Jira API v3 endpoints
#
# USAGE: 
#   .\Get-Project-Attachments-v2.ps1 -ProjectKey "ENGOPS"
#
# =============================================================================
...
param(
    ...
    [int]$MaxResults = $Params.ApiSettings.MaxResults,
    
    [Parameter(Mandatory=$false)]
    [switch]$IncludeResolved = $false
)
...
do {
    Write-Host "Fetching issues $StartAt to $($StartAt + $MaxResultsPerPage)..." -ForegroundColor Yellow
    
    # Build URL with proper encoding
    $SearchUrl = "$BaseUrl/rest/api/2/search"
    $EncodedJQL = [System.Web.HttpUtility]::UrlEncode($JQL)
    $FullUrl = "$SearchUrl`?jql=$EncodedJQL&startAt=$StartAt&maxResults=$MaxResultsPerPage&fields=key,summary,status,assignee,reporter,created,updated,attachment,issuetype,priority"
    ...
    try {
        ...
        Write-Host "  Found $IssuesInBatch issues (Total available: $TotalIssues)" -ForegroundColor Green
        
        if ($IssuesInBatch -eq 0) {
            break
        }
        ...
        Write-Host "URL attempted: $FullUrl" -ForegroundColor Gray
...
if ($AllAttachments.Count -gt 0) {
    ...
    # Top issues by attachment count
    Write-Host "TOP 10 ISSUES BY ATTACHMENT COUNT:" -ForegroundColor Cyan
    $TopIssues = $AllAttachments | Group-Object IssueKey | 
        Select-Object @{N='IssueKey';E={$_.Name}}, @{N='AttachmentCount';E={$_.Count}} |
        Sort-Object AttachmentCount -Descending
    $TopIssues | Select-Object -First 10 | Format-Table -AutoSize
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "NOTE: To download attachment content, use:" -ForegroundColor Yellow
Write-Host "  GET $BaseUrl/rest/api/3/attachment/content/{AttachmentId}" -ForegroundColor Gray
