...
# keys it's linked to through issue links. Shows ALL projects, including
...
try {
    ...
    # =============================================================================
    # PHASE 1.5: BUILD JQL QUERY FOR ALL ACTIVE PROJECTS
    # =============================================================================
    Write-Output "PHASE 1.5: Building JQL query for all active projects..."

    # Get only active projects for the query
    $ActiveProjects = $AllProjects | Where-Object { -not $_.archived }
    $ActiveProjectKeys = $ActiveProjects | ForEach-Object { $_.key }

    Write-Output "  Found $($ActiveProjects.Count) active projects to analyze"

    # Build JQL query with all active project keys (JQL has limits, so we may need to batch)
    $MaxProjectsPerQuery = 20  # Conservative limit to avoid JQL query length issues

    # =============================================================================
    # PHASE 2: GET ISSUE IDS USING ENHANCED JQL API (MINIMAL FIELDS)
    # =============================================================================
    Write-Output "PHASE 2: Getting all issue IDs using Enhanced JQL API..."

    $SearchUrl = "$BaseUrl/rest/api/3/search/jql"
    $AllIssueIds = @()
    $NextPageToken = $null
    $MaxResults = 100  # Enhanced JQL API limit
    ...
    # Process ALL projects in a single query with date restriction
    $JqlQuery = 'created >= "2020-01-01" ORDER BY created DESC'
    
    Write-Output "  Processing ALL active projects with single JQL query..."
    Write-Output "    JQL: $JqlQuery"
    ...
    Write-Output "PHASE 2 COMPLETE: Collected $($AllIssueIds.Count) issue IDs in $TotalPages pages"
    ...
    Write-Output "PHASE 3 COMPLETE: Retrieved $($AllIssueDetails.Count) issues with links"
    ...
    # Initialize only ACTIVE projects with empty link arrays
    ...
    foreach ($projectKey in ($ProjectLinks.Keys | Sort-Object)) {
        ...
        # All projects in this result are active (we only initialized active projects)
    ...
    # Export to CSV
    $OutputFile = "Issue Links - GET Project to Project Links - Anon - Official.csv"
    ...
    Write-Output "  - Active projects analyzed: $($ActiveProjects.Count)"
    ...
    if ($Result.Count -gt 0) {
        Write-Output ""
        Write-Output "Complete Project-to-Project Links Summary (Top 20):"
        Write-Output "=================================================="
        $Result | Sort-Object LinkCount -Descending | Select-Object -First 20 | Format-Table -AutoSize

        Write-Output ""
        Write-Output "Projects with most links:"
        Write-Output "========================"
