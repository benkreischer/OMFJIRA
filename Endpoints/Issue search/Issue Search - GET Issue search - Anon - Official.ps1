...
#
# DESCRIPTION: Searches for issues using JQL enhanced search with linked issues data.
# IMPLEMENTATION: Enhanced JQL API with two-phase approach (get IDs, then bulk fetch details including issue links)
...
$MaxResults = $Params.ApiSettings.MaxResults  # <-- Maximum batch size for Enhanced JQL API (reduced for better pagination)
...
foreach ($BatchIndex in 0..($Batches.Count - 1)) {
    ...
    try {
        ...
        $BatchFileName = "Issue Search - GET Issue search with Links - Anon - Official - $($StartRecord.ToString('000000'))-$($EndRecord.ToString('000000')).csv"
...
Write-Output "  - Files named: Issue Search - GET Issue search with Links - Anon - Official - XXXXXX-YYYYYY.csv"
Write-Output ""
