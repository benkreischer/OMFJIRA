# =============================================================================
# ENDPOINT: Project Types - GET Project Types
# =============================================================================
#
# API DOCUMENTATION: https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-project-types/
#
# DESCRIPTION: Returns project types using basic authentication.
# This endpoint provides access to available project types.
#
# SETUP: 
...
try {
    Write-Output "Fetching project types..."
    ...
    if ($Response -and $Response.error -eq $null) {
        # Transform each project type into a separate row
    ...
    # EXPORT TO CSV
    # =============================================================================
    $OutputFile = "Project Types - GET Project Types - Anon - Official.csv"
    $Result | Export-Csv -Path $OutputFile -NoTypeInformation -Force

    Write-Output "Wrote $OutputFile with $($Result.Count) rows."

} catch {
    Write-Error "Failed to retrieve project types data: $($_.Exception.Message)"
    exit 1
}
