...
# Create exports12 directory and cleanup
$stepExportsDir = Join-Path $outDir "exports12"
if (Test-Path $stepExportsDir) {
    Write-Host "🗑️  Cleaning up previous exports12 directory..." -ForegroundColor Yellow
...
for ($i = 0; $i -lt $exportedIssues.Count; $i += $batchSize) {
    ...
    foreach ($sourceIssue in $batch) {
        ...
        try {
            ...
            $sourceWorklogs = if ($script:DryRun) { @{ worklogs = @() } } else { Invoke-JiraWithRetry -Method GET -Uri $worklogsUri -Headers $srcHdr -MaxRetries 3 -TimeoutSec 30 }
            ...
            # ========== IDEMPOTENCY: DELETE all existing worklogs first ==========
            $existingWorklogsUri = "$($tgtBase.TrimEnd('/'))/rest/api/3/issue/$targetKey/worklog"
            $existingWorklogs = @()
            try {
                $existingWorklogsResponse = if ($script:DryRun) { @{ worklogs = @() } } else { Invoke-JiraWithRetry -Method GET -Uri $existingWorklogsUri -Headers $tgtHdr -MaxRetries 3 -TimeoutSec 30 }
            ...
            # Delete all existing worklogs for clean migration
            if ($existingWorklogs.Count -gt 0) {
                Write-Host "    🗑️  Deleting $($existingWorklogs.Count) existing worklogs for clean migration..." -ForegroundColor Yellow
                $deletedWorklogs = 0
                $failedDeletes = 0
                
                foreach ($existingWorklog in $existingWorklogs) {
                    try {
                        $deleteWorklogUri = "$($tgtBase.TrimEnd('/'))/rest/api/3/issue/$targetKey/worklog/$($existingWorklog.id)"
                        if ($script:DryRun) { Write-Host "[DRYRUN] DELETE $deleteWorklogUri" -ForegroundColor Yellow } else { Invoke-JiraWithRetry -Method DELETE -Uri $deleteWorklogUri -Headers $tgtHdr -MaxRetries 3 -TimeoutSec 30 | Out-Null }
                        $deletedWorklogs++
                    } catch {
                        $failedDeletes++
                        Write-Warning "      Failed to delete existing worklog: $($_.Exception.Message)"
                    }
                }
                
                Write-Host "    ✅ Deleted $deletedWorklogs worklogs ($failedDeletes failed)" -ForegroundColor Green
                
                # Clear existing worklogs array since we just deleted them all
                $existingWorklogs = @()
            }
            ...
            foreach ($worklog in $sourceWorklogs.worklogs) {
                ...
                try {
                    ...
                    # ========== CREATE WORKLOG ==========
                    # All existing worklogs have been deleted, so we can create fresh worklogs
                    ...
                    $response = if ($script:DryRun) { @{ id = 0 } } else { Invoke-JiraWithRetry -Method POST -Uri $createWorklogUri -Headers $tgtHdr -Body $worklogJson -ContentType "application/json" -MaxRetries 3 -TimeoutSec 30 }
...
$step12SummaryCsvPath = Join-Path $stepExportsDir "12_Worklogs_Report.csv"
...
# Generate detailed CSV report for migrated worklogs (ALWAYS CREATE)
$csvFileName = "12_Worklogs_Details.csv"
...
try {
    ...
    if ($migratedWorklogs.Count -gt 0) {
        foreach ($worklog in $migratedWorklogs) {
            $csvData += [PSCustomObject]@{
                ...
                "Time Spent (Seconds)" = $worklog.TimeSpentSeconds
                "Started" = $worklog.Started
                "Comment" = $worklog.Comment
                "Source Worklog ID" = $worklog.SourceWorklogId
                "Target Worklog ID" = $worklog.TargetWorklogId
                "Timestamp" = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
            }
        }
    } else {
        ...
        $csvData += [PSCustomObject]@{
            "Source Issue Key" = "No worklogs to migrate"
            "Target Issue Key" = ""
            "Source Issue URL" = ""
            "Target Issue URL" = ""
            "Worklog Author" = ""
    ...
    Write-Host "✅ Worklogs details report saved: $csvFileName" -ForegroundColor Green
    Write-Host "   📄 Location: $csvFilePath" -ForegroundColor Gray
    Write-Host "   📊 Worklogs migrated: $($migratedWorklogs.Count)" -ForegroundColor Gray
} catch {
    Write-Warning "Failed to save worklogs details report: $($_.Exception.Message)"
}

# Generate CSV report for failed worklogs (ALWAYS CREATE)
$failedCsvFileName = "12_Worklogs_Failed.csv"
...
try {
    ...
    if ($failedWorklogs.Count -gt 0) {
        ...
        # Create empty report with note
        $failedCsvData += [PSCustomObject]@{
            "Source Issue Key" = "No worklogs failed"
            "Target Issue Key" = ""
            "Source Issue URL" = ""
            "Target Issue URL" = ""
            "Worklog Author" = ""
    ...
    Write-Host "✅ Failed worklogs report saved: $failedCsvFileName" -ForegroundColor Green
...
Write-StageReceipt -OutDir $stepExportsDir -Stage "12_Worklogs" -Data @{
