...
function Write-StageReceipt {
  ...
  # Write timestamped file (for history/archive)
  $timestampedFile = Join-Path $OutDir ("{0}-{1}.json" -f (Get-Date -Format "yyyyMMdd_HHmmss"), $Stage)
  ($Data | ConvertTo-Json -Depth 8) | Out-File -FilePath $timestampedFile -Encoding UTF8
  
  # Also write consistent named file (for referencing by other scripts)
  $consistentFile = Join-Path $OutDir ("{0}_receipt.json" -f $Stage)
  ($Data | ConvertTo-Json -Depth 8) | Out-File -FilePath $consistentFile -Encoding UTF8
  
  Write-Host "Wrote receipt: $timestampedFile"
  Write-Host "Updated reference: $consistentFile"
