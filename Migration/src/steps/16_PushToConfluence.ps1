...
<#
.SYNOPSIS
    Step 16: Push Migration Documentation to Confluence
    
.DESCRIPTION
    This script pushes the migration documentation, receipts, and reports to Confluence
    in the JIRA space under a Sandbox > ProjectKey folder structure.
    
.PARAMETER ProjectKey
    The project key to process (e.g., QUAL)
    
.PARAMETER ParametersFile
    Path to the parameters.json file for this project
    
.PARAMETER OutDir
    Output directory containing the migration files
    
.EXAMPLE
    .\16_PushToConfluence.ps1 -ProjectKey QUAL -ParametersFile ".\projects\QUAL\parameters.json"
#>
...
function Create-ConfluencePage {
    ...
    try {
        ...
        try {
            ...
            $existingPages = Invoke-RestMethod -Uri $searchUrl -Headers $headers -Method Get
...
function Upload-ConfluenceAttachment {
    ...
    try {
        ...
        try {
            ...
            $existingAttachments = Invoke-RestMethod -Uri $existingAttachmentsUrl -Headers $headers -Method Get
    ...
    if ($projectSearchResponse.results -and $projectSearchResponse.results.Count -gt 0) {
        ...
        if ($isUnderSandbox) {
            ...
            try {
                ...
                Invoke-JiraWithRetry -Uri $deleteUrl -Headers $deleteHeaders -Method Delete -MaxRetries 3 -TimeoutSec 30
...
foreach ($pageTitle in $pagesToMove) {
    try {
        ...
        $pageSearchResponse = Invoke-RestMethod -Uri $pageSearchUrl -Headers @{
            "Authorization" = "Basic $([Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$email`:$apiToken")))"
            "Accept" = "application/json"
        } -Method Get
        ...
        if ($pageSearchResponse.results -and $pageSearchResponse.results.Count -gt 0) {
            ...
            if (-not $isUnderProject -and $isUnderSandbox) {
                ...
                $currentPageDetails = Invoke-RestMethod -Uri $getPageUrl -Headers @{
                    "Authorization" = "Basic $([Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$email`:$apiToken")))"
                    "Accept" = "application/json"
                } -Method Get
                
                # Update page to move it under QUAL
                ...
                $updatePageBody = @{
                    ...
                    body = @{
                        atlas_doc_format = @{
                            value = $currentPageDetails.body.atlas_doc_format.value
                            representation = "atlas_doc_format"
                        }
                    }
                } | ConvertTo-Json -Depth 10
                
                $updateResponse = Invoke-RestMethod -Uri $updatePageUrl -Headers @{
                    "Authorization" = "Basic $([Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$email`:$apiToken")))"
                    "Accept" = "application/json"
                    "Content-Type" = "application/json"
...
try {
    ...
    $getUrl = "$baseUrl/rest/api/content/$($projectFolder.id)?expand=version"
    $currentPage = Invoke-RestMethod -Uri $getUrl -Headers @{
        "Authorization" = "Basic $([Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$email`:$apiToken")))"
        "Accept" = "application/json"
    } -Method Get
